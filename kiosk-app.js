...
} from "./firebase-app.js?v=20260907-51";
window.liveState = liveState; // متاح عالميًا عشان أي سكريبت تاني (زي البحث في الـ nav) يقدر يقرا منه
...
function updateKioskClock() {
  ...
  if (!timeEl && !dateEl) el.textContent = timeText;
}
updateKioskClock();
...
function renderHero() {
  ...
  if(img){ img.src=logo.url||'logo(1).png'; img.style.width=`${Number(logo.size||108)}px`; img.style.height=`${Number(logo.size||108)}px`; img.style.display=logo.enabled===false?'none':''; }
...
function renderHomeGrid() {
  ...
  if (!grid) return;
  const categories = (liveState.categories || []).filter(cat => cat.visible !== false);
  ...
  fitGridColumns(grid, categories.length, 230);
}

/* ── Homepage media uses the same live images already managed by the site. ── */
function renderHomepageMedia() {
  // The homepage hero is intentionally static. Do not copy menu Hero settings into #homeHeroBg.
  const homeHeroFallback = 'https://lh3.googleusercontent.com/aida-public/AB6AXuCgJjbAdPWCs2-dx1RT_LvB802m6u7WoeiwG-e5gx8Mq3rDZq10avTfceKid9xuxm8bw-ynOwHp1nDEK-auJjnN5Ra7AJwd5mO75h4J6CSHmtYgqomcn22uYvB8WzsOHGoyA97kkdlKx5crZljvtQ9SHLT6MfUzyJqgT6w3Pr-z63Md9Vl2Kp0aDZZE7wn8pdgHI1Spjr4tOyA64IMaSpe0cwr2m3ZfuaI3cgpDx8JqqfQ7c54wMYE2BX0EFE4xLNZ4aS-cYT_BSQ';
  const categories = (liveState.categories || []).filter(cat => cat.visible !== false);
  const imageUrls = categories.map(cat => cat.imageUrl).filter(Boolean);
  const heroImage = homeHeroFallback;
  const hp = liveState.homepageSettings || {};
  const fallbackImages = [
    cldOptimize(imageUrls[0] || heroImage, 900),
    cldOptimize(imageUrls[1] || imageUrls[0] || heroImage, 900),
    cldOptimize(imageUrls[0] || heroImage, 1100),
  ];
  // The About section uses the local bakery-story image shipped with this site.
  // Keep the Firebase homepage setting available for other deployments, but prefer
  // the local file here so the section always shows the approved image.
  const chosenUrls = ['./images/%D9%85%D9%86%20%D9%86%D8%AD%D9%86.png?v=20260907-52', hp.choiceImageUrl, hp.bannerImageUrl];
  const images = chosenUrls.map((url, i) => {
    if (i === 0) return url;
    return url ? cldOptimize(url, i === 2 ? 1100 : 900) : fallbackImages[i];
  });
  ['homeAboutImage', 'homeChoiceImage', 'homeBannerImage'].forEach((id, index) => {
    const img = document.getElementById(id);
    if (!img) return;
    img.src = images[index];
    setupImageLoading(img, CAT_FALLBACK);
  });
}
